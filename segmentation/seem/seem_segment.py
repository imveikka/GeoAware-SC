...
def inference(image, text, model, transform):
    ...
    with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=use_cuda):
        ...
        mask = np.repeat(res[:,:,None], 3, axis=2)
        return np.asarray(image) * mask, mask


@torch.no_grad()
def inference_(image, text, model, transform):
    use_cuda = next(model.parameters()).is_cuda
    # device = 'cuda' if use_cuda else 'cpu'
    # dtype = torch.float16 if use_cuda else torch.float32
    with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=use_cuda):
        ...
        batch_inputs = [data]
        results,image_size,extra = model.model.evaluate_demo(batch_inputs)
    return results, image_size, extra
...
def seem_segment(img, model, transform, object="seal"):
    ...
    res, mask = inference(img, object, model, transform)
